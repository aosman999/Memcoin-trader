...
namespace cAlgo.Robots
{
    ...
    public class GoldTraderBotNews : Robot
    {
        ...
        [Parameter("Use economic calendar", DefaultValue = true, Group = "News")]
        public bool UseEconomicCalendar { get; set; }

        [Parameter("Block minutes BEFORE event", DefaultValue = 10, MinValue = 0, Group = "News")]
        public int NewsBlockBefore { get; set; }

        [Parameter("Block minutes AFTER event", DefaultValue = 15, MinValue = 0, Group = "News")]
        public int NewsBlockAfter { get; set; }

        // ---------------- internals ----------------
        private const string Label = "GoldTraderBotNews";
        ...
        private readonly List<DateTime> _highImpactEvents = new List<DateTime>();
        private readonly object _eventsLock = new object();
        private DateTime _calendarFetchedAt = DateTime.MinValue;
        private bool _calendarFetching;                      // demo-lock tripped
        ...
        private void OnMinuteBar(BarOpenedEventArgs args)
        {
            ...
            // ---- news agent: scheduled high-impact events ----------------
            if (UseEconomicCalendar)
            {
                RefreshCalendarIfStale();
                if (InNewsWindow(now))
        ...
        // ================= NEWS AGENT (economic calendar) =================
        // One read-only request per hour to the weekly calendar feed. The
        // fetch runs on a background task so the trading thread never
        // blocks; failures are silent and simply leave the cache as-is
        // (no calendar == no news blocking, never a crash).
        private void RefreshCalendarIfStale()
        {
            if (_calendarFetching || (DateTime.UtcNow - _calendarFetchedAt).TotalMinutes < 60)
                return;
            ...
            Task.Run(() =>
            {
                try
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        json = client
                            .GetStringAsync("https://nfs.faireconomy.media/ff_calendar_thisweek.json")
                            .GetAwaiter().GetResult();
                    }

                    var parsed = ParseHighImpactEvents(json);
                    lock (_eventsLock)
                    {
                        _highImpactEvents.Clear();
                        _highImpactEvents.AddRange(parsed);
                    }
                    _calendarFetchedAt = DateTime.UtcNow;
                }
                catch (Exception)
                {
                    // network hiccup: keep whatever we already had
                }
                finally
                {
        ...
        // Hand-rolled parse (no external JSON dependency): pull the "impact"
        // and "date" fields of each record and keep the High-impact ones.
        private static List<DateTime> ParseHighImpactEvents(string json)
        {
            var events = new List<DateTime>();
            if (string.IsNullOrEmpty(json))
                return events;

            var records = Regex.Matches(json, "\\{[^{}]*\\}");
            foreach (Match record in records)
            {
                var text = record.Value;
                if (text.IndexOf("\"impact\"", StringComparison.OrdinalIgnoreCase) < 0)
                    continue;
                if (text.IndexOf("High", StringComparison.OrdinalIgnoreCase) < 0)
                    continue;

                var dateMatch = Regex.Match(text, "\"date\"\\s*:\\s*\"([^\"]+)\"");
                if (!dateMatch.Success)
                    continue;

                DateTime parsed;
                // the feed is ISO-8601 with an offset, e.g. 2026-07-27T12:30:00-04:00
                if (DateTimeOffset.TryParse(dateMatch.Groups[1].Value,
                                            CultureInfo.InvariantCulture,
                                            DateTimeStyles.AssumeUniversal, out var dto))
                    parsed = dto.UtcDateTime;
                else if (!DateTime.TryParse(dateMatch.Groups[1].Value,
                                            CultureInfo.InvariantCulture,
        ...
        private bool InNewsWindow(DateTime now)
        {
            lock (_eventsLock)
            {
                foreach (var eventTime in _highImpactEvents)
                {
                    if (now >= eventTime.AddMinutes(-NewsBlockBefore) &&
                        now <= eventTime.AddMinutes(NewsBlockAfter))
                    {
                        Print("News agent: high-impact event at {0:HH:mm} UTC — standing aside.",
                              eventTime);
                        return true;
                    }
                }
